...
The [`ReactTransitionGroup`](broken-reference) add-on component is a low-level API for animation, and [`ReactCSSTransitionGroup`](broken-reference) is an add-on component for easily implementing basic CSS animations and transitions.

### High-level API: ReactCSSTransitionGroup <a href="#high-level-api-reactcsstransitiongroup" id="high-level-api-reactcsstransitiongroup"></a>

`ReactCSSTransitionGroup` is a high-level API based on [`ReactTransitionGroup`](broken-reference) and is an easy way to perform CSS transitions and animations when a React component enters or leaves the DOM. It's inspired by the excellent [ng-animate](https://docs.angularjs.org/api/ngAnimate) library.
...
#### Animate Initial Mounting <a href="#animate-initial-mounting" id="animate-initial-mounting"></a>
...
#### Custom Classes <a href="#custom-classes" id="custom-classes"></a>
...
#### Animation Group Must Be Mounted To Work <a href="#animation-group-must-be-mounted-to-work" id="animation-group-must-be-mounted-to-work"></a>
...
The example below would **not** work, because the `ReactCSSTransitionGroup` is being mounted along with the new item, instead of the new item being mounted within it. Compare this to the [Getting Started](broken-reference) section above to see the difference.
...
#### Animating One or Zero Items <a href="#animating-one-or-zero-items" id="animating-one-or-zero-items"></a>
...
#### Disabling Animations <a href="#disabling-animations" id="disabling-animations"></a>
...
### Low-level API: ReactTransitionGroup <a href="#low-level-api-reacttransitiongroup" id="low-level-api-reacttransitiongroup"></a>
...
`ReactTransitionGroup` is the basis for animations. When children are declaratively added or removed from it (as in the [example above](broken-reference)), special lifecycle methods are called on them.

- [`componentWillAppear()`](broken-reference)
- [`componentDidAppear()`](broken-reference)
- [`componentWillEnter()`](broken-reference)
- [`componentDidEnter()`](broken-reference)
- [`componentWillLeave()`](broken-reference)
- [`componentDidLeave()`](broken-reference)
...
#### `componentWillAppear()` <a href="#componentwillappear" id="componentwillappear"></a>
...
#### `componentDidAppear()` <a href="#componentdidappear" id="componentdidappear"></a>
...
#### `componentWillEnter()` <a href="#componentwillenter" id="componentwillenter"></a>
...
#### `componentDidEnter()` <a href="#componentdidenter" id="componentdidenter"></a>

```javascript
componentDidEnter();
```

This is called after the `callback` function that was passed to [`componentWillEnter()`](broken-reference) is called.

---

#### `componentWillLeave()` <a href="#componentwillleave" id="componentwillleave"></a>
...
#### `componentDidLeave()` <a href="#componentdidleave" id="componentdidleave"></a>
